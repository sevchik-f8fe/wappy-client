/**
 * Хуки для управления шапкой приложения и действиями пользователя
 * 
 * Хук usePannel предоставляет функции:
 * - deleteAction: Удаление аккаунта пользователя (требуется аутентификация)
 * - logOutAction: Выход из системы (очистка Redux-состояния)
 * - logInAction: Перенаправление на страницу входа
 * - changeEmailAction: Перенаправление на страницу смены email
 * 
 * Особенности:
 * - Использует Redux для управления состоянием пользователя и токена
 * - Требует авторизацию для sensitive операций (удаление, смена email)
 * - Встроенная обработка ошибок с toast-уведомлениями
 * - Автоматическая очистка состояния при выходе/удалении
 */
