...
_how the search works_: the search queries are sent to the server in parallel. the returned results are checked and shuffled.
...
**Key features**: two-factor authentication, registration, confirmation when changing email (Yandex SMTP), session storage encryption (absolutely all your data), token stored in axios, favorites, history of load, ~~inability to directly download images from Wallhaven~~:)
